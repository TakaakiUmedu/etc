...
int main(void){
	...
	const auto to_int = [N, M](int64_t v1, int64_t v2, int64_t s){
		if(v1 > v2){
			swap(v1, v2);
		}
		return (((v1 * N) + v2) << M) + s;
	...
	const auto to_values = [N, M](int64_t x){
		const auto s = x & ((1 << M) - 1);
		x >>= M;
		const auto v2 = x % N;
		x /= N;
		return make_tuple(x, v2, s);
	};
...
//	print(flows_tmp);
	...
	const auto update = [&to_int, &flows_tmp, &best, T](vector<int64_t>& amounts, int64_t v1, int64_t v2, int64_t s, int64_t a, int64_t t){
		const auto x = to_int(v1, v2, s);
		if(x >= amounts.size()){
...
//				push_heap(queue.begin(), queue.end());
	...
	vector<int64_t> amounts((1 << M) * N * N, -1);
	update(amounts, S, S, 0, 0, 0);
	
	for(const auto t: range(T)){
		print(t);
		vector<int64_t> amounts_tmp((1 << M) * N * N, -1);
		for(const auto v1: range(N)){
			for(const auto v2: range(v1, N)){
					...
					if(a >= 0){
						{
							...
							if(s1 != 0){
								...
								update(amounts_tmp, v1, v2, s1, a1, t + 1);
								for(const auto n: graph[v2]){
									update(amounts_tmp, v1, n, s1, a1, t + 1);
								}
								const auto s3 = open(v2, s1);
								if(s3 != 0){
									const auto a3 = a1 + (T - t - 1) * flows[v2];
									update(amounts_tmp, v1, v2, s3, a3, t + 1);
								}
							}
						}
						{
							...
							if(s2 != 0){
								...
								for(const auto n: graph[v1]){
									update(amounts_tmp, n, v2, s2, a2, t + 1);
								}
							}
						}
						for(const auto n1: graph[v1]){
							for(const auto n2: graph[v2]){
								update(amounts_tmp, n1, n2, s, a, t + 1);
							}
						}
					}
				}
