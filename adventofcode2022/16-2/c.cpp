...
//	print(flows_tmp);
	...
	const auto push = [&best, &amounts, &queue, &to_int, &flows_tmp, T](int64_t v1, int64_t v2, int64_t t, int64_t s, int64_t a){
		const auto x = to_int(v1, v2, t, s);
		if(x >= amounts.size()){
			print("error", x, v1, v2, t, s);
			exit(0);
		}
