...
int main(void){
	...
	for(const auto x: range(max_x + 2)){
		for(const auto y: range(max_y + 2)){
			for(const auto z: range(max_z + 2)){
				if(grid[x][y][z]){
					if(grid[x + 1][y][z]){
						total -= 2;
					}
	...
	while(queue.size() > 0){
		...
		for(const auto& [dx, dy, dz]: ds){
			...
			if(nx >= 0 && nx < max_x + 3 && ny >= 0 && ny < max_y + 3 && nz >= 0 && nz < max_z + 3){
				if(!visited[nx][ny][nz] && !grid[nx][ny][nz]){
	...
	cout << total - count * 6 + walls * 2 << endl;
