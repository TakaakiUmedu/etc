...
inline tuple<int16_t, int16_t, int16_t> add(const tuple<int16_t, int16_t, int16_t>& a, const tuple<int8_t, int8_t, int8_t, int8_t>& b){
	return make_tuple(get<0>(a) + get<0>(b), get<1>(a) + get<1>(b), get<2>(a) + get<2>(b));
}

inline tuple<int16_t, int16_t, int16_t> sub(const tuple<int16_t, int16_t, int16_t>& a, const tuple<int64_t, int64_t, int64_t>& b){
	return make_tuple(get<0>(a) - get<0>(b), get<1>(a) - get<1>(b), get<2>(a) - get<2>(b));
}

inline void insert(set<tuple<int16_t, int16_t, int16_t>>& items, const tuple<int16_t, int16_t, int16_t>& it){
	auto p = items.begin();
	while(p != items.end()){
		const auto it2 = *p;
		...
		if(get<0>(it2) <= get<0>(it) && get<1>(it2) <= get<1>(it) && get<2>(it2) <= get<2>(it)){
			p = items.erase(p);
		}else{
			p ++;
		}
...
//	const auto X = 100;
	...
	for(const auto& blueprint: blueprint_table){
		cout << "<" << t << ">" << endl;
		map<tuple<int8_t, int8_t, int8_t, int8_t>, set<tuple<int16_t, int16_t, int16_t>>> states;
		states[make_tuple(1, 0, 0, 0)].insert(make_tuple(0, 0, 0));
		for(const auto i: range(L)){
			map<tuple<int8_t, int8_t, int8_t, int8_t>, set<tuple<int16_t, int16_t, int16_t>>> new_states;
	...
	//					print(rt, it);
					...
					for(const auto& [recepie, d]: blueprint){
						...
						if(get<0>(new_items2) >= 0 && get<1>(new_items2) >= 0 && get<2>(new_items2) >= 0){
...
//							new_states.insert(make_tuple(composer1.compose(get<0>(new_rt), get<1>(new_rt), get<2>(new_rt), get<3>(new_rt)), composer2.compose(get<0>(new_it3), get<1>(new_it3), get<2>(new_it3), get<3>(new_it3))));
			...
			int16_t m = 0;
			for(const auto& [robots, items_list]: states){
				m = max(m, static_cast<int16_t>(get<3>(robots)));
			}
			{
				auto p = states.begin();
				if(p != states.end()){
					const auto& robots = p->first;
					const auto m2 = static_cast<int16_t>(get<3>(robots) + (L - i) * (L - i + 1) / 2);
					if(m2 < m){
						p = states.erase(p);
					}else{
						p ++;
