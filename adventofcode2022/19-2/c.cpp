...
inline void insert(const composer<int64_t, int64_t, int64_t, int64_t, int64_t>& composer2, set<int64_t>& items, const tuple<int64_t, int64_t, int64_t, int64_t>& it){
	auto p = items.begin();
	while(p != items.end()){
		const auto it2 = composer2.decompose(*p);
	...
	items.insert(composer2.compose(get<0>(it), get<1>(it), get<2>(it), get<3>(it)));
}
...
int main(void){
	...
	for(const auto& blueprint: blueprint_table){
		cout << "<" << t << ">" << endl;
		map<int32_t, set<int64_t>> states;
		states[composer1.compose(1, 0, 0, 0)].insert(0L);
		for(const auto i: range(32)){
			map<int32_t, set<int64_t>> new_states;
...
//					new_states.insert(make_tuple(robots, composer2.compose(get<0>(new_it), get<1>(new_it), get<2>(new_it), get<3>(new_it))));
					insert(composer2, new_states[robots], new_it);
...
//							new_states.insert(make_tuple(composer1.compose(get<0>(new_rt), get<1>(new_rt), get<2>(new_rt), get<3>(new_rt)), composer2.compose(get<0>(new_it3), get<1>(new_it3), get<2>(new_it3), get<3>(new_it3))));
							insert(composer2, new_states[composer1.compose(get<0>(new_rt), get<1>(new_rt), get<2>(new_rt), get<3>(new_rt))], new_it3);
						}
					}
				}
			}
