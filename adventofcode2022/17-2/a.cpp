...
public:
	...
	void print(int64_t count = -1) const{
		int64_t c = 0;
		for(const auto j: range(field.size() - 1, -1, -1)){
//		for(const auto j: range(field.size())){
...
bool check(vector<tuple<int64_t, int, int, int>>& data, size_t s1, size_t e1, size_t s2, size_t e2){
	if(e1 - s1 != e2 - s2 || s1 == s2){
		return false;
	}
//	print("checking", make_tuple(s1, e1), make_tuple(s2, e2));
	for(const auto i: range(s1, e1)){
		const auto j = i - s1 + s2;
		if(data[i] != data[j]){
//			print("failed at", i - s1);
			return false;
		}
	}
	return true;
...
//	field.add_line("");
	...
	vector<tuple<int64_t, int, int, int>> data;
	vector<int64_t> heights;
	
	map<tuple<int64_t, int, int, int>, vector<size_t>> indexes;
	
	size_t s1, e1, s2, e2;
	
	int64_t prev_offset = -100;
	bool found = false;
	heights.push_back(field.height() - 1);
	for(const auto i: range(N)){
		...
		int64_t offset = field.height() + 3;
		while(true){
			const auto m = moves[j];
			j ++;
			if(j >= moves.size()){
				j = 0;
			}
			...
			if(field.check(b, offset - 1)){
				...
				field.copy(b, offset);
				const auto d = make_tuple(prev_offset - offset, b.x, k, j);
				prev_offset = offset;
				auto p = indexes.find(d);
				if(p != indexes.end()){
					auto& index_list = p->second;
					index_list.push_back(data.size());
					if(index_list.size() >= 3){
						for(const auto i0: range(index_list.size())){
							s1 = index_list[i0];
							for(const auto j0: range(i0 + 1, index_list.size())){
								e1 = index_list[j0];
								for(const auto i1: range(index_list.size())){
									s2 = index_list[i1];
									for(const auto j1: range(i1 + 1, index_list.size())){
										e2 = index_list[j1];
										if(check(data, s1, e1, s2, e2)){
											found = true;
											break;
										}
									}
...
//	for(const auto i: range(s1, e1 + 1)){
//		print(data[i]);
//	}
	
//	print(indexes);
	print(found, s1, e1, s2, e2);
	print(found, e1 - s1);
	print(heights.size(), N, field.height() - 1);
	
	if(heights.size() > N){
		cout << heights[N] << endl;
	}else{
		const auto n = s1 + (N - s1) % (e1 - s1);
		cout << heights[n] + (heights[e1] - heights[s1]) * ((N - s1) / (e1 - s1)) << endl;
	}
/*
	const auto N = read<int64_t>();
